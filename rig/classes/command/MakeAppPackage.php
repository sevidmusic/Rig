...
#        }
    ...
    {
        $this->validateMakeShCallsToRigNewApp($preparedArguments);
        $this->validateMakeShIsExecutable($preparedArguments);
        $this->validateMakeShsCallToRigNewAppUseCorrectName($preparedArguments);
    }
     ...
     * @param array{"flags": array<string, array<int, string>>, "options": array<int, string>} $preparedArguments
     */
    private function validateMakeShCallsToRigNewApp(array $preparedArguments): void
    {
        $makeShContent = $this->getMakeShContent($preparedArguments);
        $callsToRigNewApp = substr_count($makeShContent, 'rig --new-app');
        if($callsToRigNewApp !== 1) {
            switch($callsToRigNewApp < 1) {
                case true:
                    throw new RuntimeException(
                        '  The specified App Package\'s make.sh is not valid' . PHP_EOL .
                        '  because it does not define a call to `rig --new-app`.' . PHP_EOL . PHP_EOL .
                        '  An App Package\'s make.sh MUST define exactly one call to `rig --new-app`.' . PHP_EOL .
                        '  App Packages MUST create an App, and MUST NOT create more than one App.' . PHP_EOL
                    );
                default:
                    throw new RuntimeException(
                        '  The specified App Package\'s make.sh is not valid' . PHP_EOL .
                        '  because it defines more than one call to `rig --new-app`.' . PHP_EOL . PHP_EOL .
                        '  An App Package\'s make.sh MUST define exactly one call to `rig --new-app`.' . PHP_EOL .
                        '  App Packages MUST create an App, and MUST NOT create more than one App.' . PHP_EOL
                    );
            }
     ...
     * @param array{"flags": array<string, array<int, string>>, "options": array<int, string>} $preparedArguments
     */
    private function validateMakeShsCallToRigNewAppUseCorrectName(array $preparedArguments): void
    {
        $makeShContent = $this->getMakeShContent($preparedArguments);
        $expectedName = $this->determineAppName($preparedArguments);
        if(!str_contains($makeShContent, 'rig --new-app --name ' . $expectedName)) {
